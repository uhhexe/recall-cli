...
type Config struct {
	...
	RecallApiKey     string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("RECALL_API_KEY"); v != "" {
		cfg.RecallApiKey = v
		cfg.markEnvOverride("RecallApiKey")
		cfg.AuthSource = "env:RECALL_API_KEY"
		cfg.CredentialSource = "env:RECALL_API_KEY"
	}
	...
	if v := os.Getenv("RECALL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("RECALL_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "recall", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":          token,
		"api_key":        c.RecallApiKey,
		"RECALL_API_KEY": c.RecallApiKey,
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.RecallApiKey = creds.RecallApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.RecallApiKey = token
	delete(c.envOverrides, "RecallApiKey")
	c.updateFileConfigField("RecallApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RecallApiKey"] {
			out.RecallApiKey = c.fileConfig.RecallApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RecallApiKey":
		c.fileConfig.RecallApiKey = c.RecallApiKey
	}
}
