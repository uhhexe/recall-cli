...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  recall doctor
  recall doctor --json
  recall doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export RECALL_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("RECALL_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "RECALL_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RECALL_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "recall", suggestion)
						} else {
			...
			report["region"] = resolvedRegion(flags.region)
			...
			for _, key := range []string{"config_path", "base_url", "region", "auth_source", "credentials_location", "version"} {
	...
	cmd.Flags().StringVar(&failOn, "fail-on", "", "Exit non-zero for selected health gates. warn: credential/path warnings plus errors; error: errors only. Default is never.")
	return cmd
}
...
// trips on deliberate WARN sections plus errors. The default empty string
// means never fail on status.
